...
RUN apt-get update && apt-get install -y --no-install-recommends xz-utils man-db mc mcedit vim apt-utils curl
# nettoyage apt
...
# docker build --no-cache -t benevfront_i .
# docker run -d --name benevfront_c -p 5173:5173 benevfront_i:latest