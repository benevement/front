...
const VolunteerTask = () => {
    ...
    //const role = useAuthStore((state) => (state.user?.role)); TODO: à réactiver
    const role="volunteer" // test faker
//const token = useAuthStore((state) => (state.token)); // TODO: voir pour récup le Token suite aux modifs
...
const event_address = fakeAddress.find((a) => (a.id === event?.address_id)); //TODO: adapter
        ...
        <div className={isVolunteer ? "" : "hidden"}>
            <div className="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8 border-2 border-blue-400">
                <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm border-2 border-amber-100">
                    ...
                    <div className="bg-cyan-400/10 h-10 font-black">{event?.name && firstLetterUpper(event.name)}</div>
